...
import BlockchainService, { CandidateStatusEnum, ElectionStateEnum } from '../../services/BlockchainService';
...
const ElectionManagement = () => {
  ...
  const [endTime, setEndTime] = useState(new Date(Date.now() + 24 * 60 * 60 * 1000)); // Default to 1 day in the future
  ...
  const toggleCandidateSelection = (candidateAddress) => {
    if (selectedCandidates.includes(candidateAddress)) {
      setSelectedCandidates(selectedCandidates.filter(address => address !== candidateAddress));
    } else {
  ...
  const handleCreateElection = async (e) => {
    ...
    try {
      ...
      setFormError(`Failed to create election: ${error.message || 'Unknown error'}`);
    } finally {
  ...
  const handleStartElection = async (electionId) => {
    ...
    try {
      ...
      if (errorMessage.includes("Internal JSON-RPC error")) {
        errorMessage = "Transaction failed. There might be an issue with the blockchain transaction.";
      }
      
      setFormError(`Failed to start election: ${errorMessage}`);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleEndElection = async (electionId) => {
    ...
    try {
      ...
      setFormError(`Failed to end election: ${error.message || 'Unknown error'}`);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleResetContract = async () => {
    ...
    try {
      ...
      setFormError(`Failed to reset contract: ${error.message || 'Unknown error'}`);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleRefreshElections = async () => {
    ...
    try {
      ...
      setFormError(`Failed to refresh elections: ${error.message || 'Unknown error'}`);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  if (!isAdmin) {
    return (
      <div className="admin-access-denied">
        <h2>Access Denied</h2>
        <p>You do not have permission to access the election management panel.</p>
  ...
  return (
    <div className="election-container">
      <div className="election-header">
        <img src={assets.logo} alt="CarthageChain Logo" className="election-logo" />
      ...
      {resetConfirmation && (
        <div className="confirmation-modal">
          <div className="confirmation-content">
            <h3>Confirm Reset</h3>
            <p>This will remove all approved candidates and voters from the system. This action cannot be undone.</p>
      ...
      <div className="manual-control-info">
        <p>Elections require manual start and end by an administrator. The dates are used for reference only.</p>
      </div>
            ...
            <div className="form-group">
              ...
              <div className="candidates-list">
                {approvedCandidates.length === 0 ? (
                  <p className="no-candidates">No approved candidates available.</p>
                ) : (
                  approvedCandidates.map((candidate, index) => (
                    <div 
                      key={index} 
                      className={`candidate-item ${selectedCandidates.includes(candidate.address) ? 'selected' : ''}`}
                      onClick={() => toggleCandidateSelection(candidate.address)}
                    >
                      <div className="candidate-info">
                        <h3>{candidate.fullName}</h3>
                        <p>Party: {candidate.politicalParty}</p>
                        <p className="candidate-address">{candidate.address.substring(0, 6)}...{candidate.address.slice(-4)}</p>
                      </div>
                      <div className="candidate-selection">
                        <input
                          type="checkbox"
                          checked={selectedCandidates.includes(candidate.address)}
                          onChange={() => {}}
                        />
                      </div>
                    </div>
              ...
              <p className="selection-info">Selected: {selectedCandidates.length} candidates</p>
            </div>
        ...
        {allElections.length === 0 ? (
          ...
          <div className="elections-list">
            {allElections.map((election, index) => {
              return (
                <div key={index} className={`election-item state-${getStateClass(election.state)}`}>
                  <div className="election-item-header">
                    <h3>{election.name}</h3>
                    <span className={`election-state state-${getStateClass(election.state)}`}>
